Shader "Custom/TerrainLitSplat"
{
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			TestOutput TestVert(Attributes v)
			{
				TestOutput o;
				o.clipPos = TransformObjectToHClip(v.positionOS + v.normalOS * 0.1);

				o.uvMainAndLM.xy = v.texcoord;

				float2 splatUV = (v.texcoord.xy * (_Control_TexelSize.zw - 1.0f) + 0.5f) * _Control_TexelSize.xy;
            	
            	// float2 uv = TRANSFORM_TEX(v.texcoord, _Control);
            	o.splatControl = SAMPLE_TEXTURE2D_LOD(_Control, sampler_Control, splatUV, 0);
				
				return o;
			}

            half4 TestFrag(TestOutput IN) : SV_TARGET
            {
				// float2 splatUV = (IN.uvMainAndLM.xy * (_Control_TexelSize.zw - 1.0f) + 0.5f) * _Control_TexelSize.xy;
				// half4 splatControl = SAMPLE_TEXTURE2D(_Control, sampler_Control, splatUV);
	   //
				return  IN.splatControl;
            	// return lerp(float4(0,0,0,1),float4(1,1,1,1), IN.splatControl.r);
